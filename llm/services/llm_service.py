...
class LLMService:
    ...
    async def generate_thinking_directions(
        ...
        system_prompt = """Ты аналитик коллективного интеллекта. Анализируй решения по СУТИ, не по форме.
    ...
    async def generate_collective_ideas(
        ...
        system_prompt = """Ты креативный синтезатор коллективного интеллекта. Комбинируй полезные элементы из разных решений.

            Элементы: методы, техники, подходы, метафоры, опыт, инсайты, наблюдения.
            
            ТРЕБОВАНИЯ:
            1. idea_description - 2-3 предложения, ТОЛЬКО суть идеи
    ...
    async def generate_improvement_suggestions(
        ...
        system_prompt = """Ты консультант по улучшению решений. Анализируй решение и предлагай конкретные улучшения из опыта других.

            ТРЕБОВАНИЯ К source_examples:
            - ТОЧНЫЕ идеи из решений, 2-3 предложения
            - ОБЕЗЛИЧЕНЫ: без номеров решений, авторов, ID
            - Конкретные подходы/методы, не общие фразы
    ...
    async def generate_solution_criticism(
        ...
        system_prompt = """Ты критический аналитик решений. Находишь слабые места через сравнение с другими подходами.

            ТРЕБОВАНИЯ К reasoning:
            - ОБЕЗЛИЧЕННЫЕ формулировки
            - Без номеров решений, авторов, ID
            - Фокус на методологических различиях
        ...
        try:
            ...
            for direction_data in directions_data:
                direction = ThinkingDirection(
                    ...
                    participants_count=direction_data.get("participants_count", 1),
        ...
        try:
            ...
            for idea_data in ideas_data:
                idea = CollectiveIdea(
                    ...
                    source_solutions_count=idea_data.get("source_solutions_count", 0),
